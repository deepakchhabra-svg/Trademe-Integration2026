...
function getGenericMock<T>(path: string): T {
  if (path.includes("/internal-products/")) return { id: 201, sku: "TEST-SKU", title: "Test Product", supplier_product: null, cost_price: 100, images: [] } as any;
  if (path.includes("/supplier-products/")) return { id: 101, external_sku: "TEST-SKU", title: "Test Product", cost_price: 100, images: [], specs: {} } as any;
  if (path.includes("/listings/")) return { id: 301, internal_product_id: 201, title: "Test Listing", price: 120, status: "DRY_RUN", images: [] } as any;
  if (path.includes("/vaults/enriched") && !path.includes("/vaults/enriched/")) return { items: [{ id: 201, sku: "TEST-SKU", title: "Test Enriched", enrichment_status: "SUCCESS" }], total: 1 } as any;
  if (path.includes("/vaults/live") && !path.includes("/vaults/live/")) return { items: [{ id: 301, internal_product_id: 201, title: "Test Live", status: "LIVE", price: 150 }], total: 1 } as any;
  if (path.includes("/vaults/raw") && !path.includes("/vaults/raw/")) return { items: [{ id: 101, external_sku: "TEST-SKU", title: "Test Raw", sync_status: "SYNCED" }], total: 1 } as any;
  if (path.includes("/trust/")) return { score: 95, is_trusted: true, blockers: [], breakdown: {} } as any;
  if (path.includes("/validate/")) return { ok: true, reason: null } as any;
  if (path.includes("/suppliers") && !path.includes("/suppliers/")) return [{ id: 1, name: "ONECHEQ", is_active: true }] as any;
  if (path.includes("/suppliers/")) return { id: 1, name: "ONECHEQ", policy: { scrape: { category_presets: ["smartphones"] } } } as any;
  ...
  if (path.includes("/commands/")) return { id: "1", type: "TEST", status: "SUCCEEDED" } as any;
  if (path.includes("/settings/")) {
    const parts = path.split("/");
    const key = parts[parts.length - 1];
    return { key, value: null, updated_at: null } as any;
  }
  if (path.includes("/ops/summary")) return { commands: { pending: 0, executing: 0, human_required: 0, failed: 0 }, vaults: { raw_total: 0, raw_present: 0, enriched_total: 0, enriched_ready: 0, listings_total: 0, listings_dry_run: 0, listings_live: 0 }, orders: { total: 0, pending_fulfillment: 0 } } as any;
  if (path.includes("/ops/alerts")) return { alerts: [{ severity: "high", code: "TEST", title: "Test Alert", detail: "Test Detail" }], count: 1 } as any;
  if (path === "/health" || path.includes("/health")) return { status: "ok", utc: new Date().toISOString() } as any;
  if (path === "/whoami" || path.includes("/whoami")) return { role: "root", rank: 100 } as any;
  return {} as T;
}

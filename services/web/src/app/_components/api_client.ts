...
function getGenericMockClient<T>(path: string): T {
  if (path.includes("/policy")) return { policy: { scrape: { category_presets: ["smartphones", "laptops"] } } } as any;
  if (path === "/suppliers") return [{ id: 1, name: "ONECHEQ", is_active: true }] as any;
  if (path.includes("/suppliers/")) return { id: 1, name: "ONECHEQ" } as any;
  return {} as T;
}
